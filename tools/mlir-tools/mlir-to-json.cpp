...
static cl::opt<std::string> databasePath(cl::Positional,
...
class ToJson {
   ...
   nlohmann::json convertType(mlir::Type type){
      return llvm::TypeSwitch<mlir::Type, nlohmann::json>(type)
         ...
         .Default([](mlir::Type type){
            llvm::errs() << "type could not be converted ";type.dump(); llvm::errs() << "\n";
            return nlohmann::json();
         });
   }
   ...
   nlohmann::json convertConstant(mlir::Attribute attr, mlir::Type type){
      ...
      if (auto integerAttr = attr.dyn_cast_or_null<mlir::IntegerAttr>()){
        ...
        llvm::errs() << "Constant could not be converted "; attr.dump();
      }
      return result;
   }
   ...
   nlohmann::json convertExpression(mlir::Operation* operation){
      return llvm::TypeSwitch<mlir::Operation*, nlohmann::json>(operation)
         .Case<mlir::tuples::ReturnOp>([&](mlir::tuples::ReturnOp returnOp){
            if (returnOp->getOperands().size()) convertExpression(returnOp.getOperand(0).getDefiningOp());
            return nlohmann::json{};
         })
         ...
         .Case<mlir::db::BetweenOp>([&](mlir::db::BetweenOp betweenOp){
           return nlohmann::json{
              {"expression", "between"},
              {"input", {
                      convertExpression(betweenOp.getOperand(0).getDefiningOp()),
                      convertExpression(betweenOp.getLower().getDefiningOp()),
                      convertExpression(betweenOp.getUpper().getDefiningOp())
                 }
              }
           };
         })
         ...
         .Case<mlir::scf::IfOp>([&](mlir::scf::IfOp ifOp){
            return nlohmann::json{
               ...
               {"cases", nlohmann::json::array({
                     {
                        {"condition", convertExpression(ifOp.getOperand().getDefiningOp())},
                        {"result", convertExpression(ifOp.getThenRegion().getBlocks().begin()->getTerminator())}
                     }
                  })
               },
         ...
         .Default([](mlir::Operation* op){
            llvm::errs() << "the following expression could not be converted"; op->dump();
            return nlohmann::json();
         });
   }
   ...
   nlohmann::json convertSort(nlohmann::json result, mlir::Operation* input, mlir::ArrayAttr sortspecs, std::optional<uint32_t> maxRows = std::nullopt){
      ...
      for (auto attr : sortspecs){
         ...
         result["order"].push_back(nlohmann::json{
            {"value", {
                   {"expression", "iuref"},
                   {"iu", symbolRefToIuString(sortspecAttr.getAttr().getName())}
                }
            },
            {"collate", ""}
         });
   ...
   nlohmann::json convertOperation(mlir::Operation* op){
      ...
      return llvm::TypeSwitch<mlir::Operation*, nlohmann::json>(op)
         ...
         .Default([&](mlir::Operation* op){
            llvm::errs() << "The following operation could not be converted ";op->dump();
