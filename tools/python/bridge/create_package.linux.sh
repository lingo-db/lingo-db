...
venv/bin/python3 -m pip install build pyarrow===22.0.0
...
${MLIR_BIN_DIR}/mlir-tblgen -gen-python-op-bindings -bind-dialect=pyinterp -I ${MLIR_INCLUDE_DIR} -I  ${BASE_PATH}/include/ dialects/PyInterp.td > src/lingodbbridge/mlir/dialects/_pyinterp_ops_gen.py
...
auditwheel repair dist/*.whl --plat "$PLAT" --exclude libarrow_python.so.2200 --exclude libarrow.so.2200 -w /built-packages
