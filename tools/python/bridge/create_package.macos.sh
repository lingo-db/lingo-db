...
# Build Arrow if not already installed or version is not 23.0.0
if [ ! -d "$ARROW_INSTALL_DIR" ] || [ ! -f "$ARROW_INSTALL_DIR/lib/cmake/Arrow/ArrowConfig.cmake" ] || [ "$(grep -Eo 'PACKAGE_VERSION[[:space:]]+"[0-9]+\.[0-9]+\.[0-9]+"\)' $ARROW_INSTALL_DIR/lib/cmake/Arrow/ArrowConfigVersion.cmake | grep -Eo '[0-9]+\.[0-9]+\.[0-9]+')" != "23.0.0" ]; then
  ...
  echo "Arrow version 23.0.0 is already installed. Skipping build."
fi

/opt/homebrew/bin/python3 -m venv $BASE_PATH/build/venv
$BASE_PATH/build/venv/bin/python3 -m pip install build pyarrow===23.0.0
...
$BASE_PATH/build/venv/bin/delocate-wheel -v dist/*.whl -e libarrow_python.2300.dylib -e libarrow.2300.dylib -w ./build-packages --ignore-missing-dependencies
