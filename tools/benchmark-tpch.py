...
for qnum in range(1, 22):
    ...
    proc1 = subprocess.run(
        "timeout 5 python3 ./tools/sql-to-mlir/sql-to-mlir.py " + file1 + " | " + objdir + "/mlir-db-opt --relalg-extract-nested-operators --relalg-decompose-lambdas --relalg-implicit-to-explicit-joins --relalg-pushdown --relalg-unnesting --relalg-optimize-join-order --relalg-combine-predicates --relalg-optimize-implementations | " + objdir + "/mlir-db-opt -relalg-to-db -canonicalize | " + objdir + "/db-run \"-\" resources/data/tpch-1",
