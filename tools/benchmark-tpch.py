...
for qnum in range(1, 22):
    ...
    proc1 = subprocess.run(
        "python3 ./tools/sql-to-mlir/sql-to-mlir.py " + file1 + " | " + objdir + "/mlir-db-opt --relalg-extract-nested-operators --cse --canonicalize --relalg-decompose-lambdas --canonicalize --relalg-implicit-to-explicit-joins --relalg-pushdown --relalg-unnesting --relalg-optimize-join-order --relalg-combine-predicates --relalg-optimize-implementations  --relalg-introduce-tmp | " + objdir + "/mlir-db-opt -relalg-to-db -canonicalize | timeout 10 " + objdir + "/db-run \"-\" resources/data/tpch-1",
