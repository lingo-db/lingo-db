...
def main():
    ...
    # Command 1: Run SQL
    command_1 = f'env {" ".join([f"{k}={v}" for k, v in env_vars.items()])} {bin_dir}/run-mlir {query} {data_dir}'
    error_list = []
    try:
        run_command(command_1)
    except subprocess.CalledProcessError as e:
        print(f"Error when running query ({command_1}), switching to debug mode")
        print("STDOUT:")
