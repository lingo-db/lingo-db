...
class List {
   ...
   public:
   List(size_t sizeOfType, StorageClass storageClass = StorageClass::REFCOUNTED) : sizeOfType(sizeOfType), len(), values(2 * sizeOfType), storageClass(storageClass) {}
   ...
   static List* promoteToGlobal(List* list);
};
