...
TEST_CASE("Length:SingleByte") {
   ...
   str = VarLen32::fromString("Hello, world!");
   REQUIRE(StringRuntime::len(str) == 13);

   str = VarLen32::fromString("DON'T PANIC");
   REQUIRE(StringRuntime::len(str) == 11);

   str = VarLen32::fromString("Time is an illusion. Lunchtime doubly so.");
   REQUIRE(StringRuntime::len(str) == 41);
}
