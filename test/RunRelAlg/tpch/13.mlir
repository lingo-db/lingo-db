...
module {
  func @main() -> !db.table {
    ...
    %2 = relalg.outerjoin @oj0 %0, %1 (%arg0: !relalg.tuple){
      %7 = relalg.getattr %arg0 @customer::@c_custkey : i32
      %8 = relalg.getattr %arg0 @orders::@o_custkey : i32
      %9 = db.compare eq %7 : i32, %8 : i32
      %10 = relalg.getattr %arg0 @orders::@o_comment : !db.string
      %11 = db.constant("%special%requests%") : !db.string
      %12 = db.compare like %10 : !db.string, %11 : !db.string
      %13 = db.not %12 : i1
      %14 = db.and %9:i1,%13:i1
      relalg.return %14 : i1
    }  mapping: {@o_shippriority({type = !db.nullable<i32>})=[@orders::@o_shippriority], @o_orderpriority({type = !db.nullable<!db.string>})=[@orders::@o_orderpriority], @o_orderdate({type = !db.nullable<!db.date<day>>})=[@orders::@o_orderdate], @o_clerk({type = !db.nullable<!db.string>})=[@orders::@o_clerk], @o_totalprice({type = !db.nullable<!db.decimal<15, 2>>})=[@orders::@o_totalprice], @o_orderstatus({type = !db.nullable<!db.char<1>>})=[@orders::@o_orderstatus], @o_comment({type = !db.nullable<!db.string>})=[@orders::@o_comment], @o_custkey({type = !db.nullable<i32>})=[@orders::@o_custkey], @o_orderkey({type = !db.nullable<i32>})=[@orders::@o_orderkey]}
