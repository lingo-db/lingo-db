 ...
 module {
	...
	func private @get_table(memref<i8>, memref<i8>, index) -> memref<i8>
	func private @get_column_id(memref<i8>, memref<i8>, index) -> index
	...
	func private @table_chunk_num_rows(memref<i8>) -> index
    ...
    //CHECK: int(5001)
 	func @main (%execution_context: memref<i8>) {
		...
		%hoeren_table = call  @get_table(%execution_context, %30, %10) : (memref<i8>, memref<i8>, index) -> memref<i8>
		%matrnr_id =  call @get_column_id(%hoeren_table,%31,%11) : (memref<i8>, memref<i8>, index) -> index
		%vorlnr_id =  call @get_column_id(%hoeren_table,%32,%12) : (memref<i8>, memref<i8>, index) -> index
		%table_chunk_iterator = call @table_chunk_iterator_init(%hoeren_table) : (memref<i8>) -> memref<i8>
   		%res = scf.while (%arg1 = %table_chunk_iterator) : (memref<i8>) -> (memref<i8>) {
    		...
    		^bb0(%arg2: memref<i8>):
    			...
    			%num_rows = call @table_chunk_num_rows(%curr_chunk) : (memref<i8>) -> index
				...
				scf.for %iv = %c0 to %num_rows step %c1 {
