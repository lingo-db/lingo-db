...
module @querymodule  {
  func @query() {
    ...
    //CHECK: %{{.*}} = relalg.const_relation @constrel2  attributes : [@attr1({type = i32})] values : [1, 2]
  	...
  	%1 = relalg.const_relation @constrel2  attributes : [@attr1({type = i32})] values : [1, 2]
...
module @querymodule  {
  func @query() {
    //CHECK: %{{.*}} = relalg.const_relation @constrel  attributes : [@attr1({type = i32})] values : [1, 2]
    //CHECK: %{{.*}} = relalg.const_relation @constrel2  attributes : [@attr1({type = i32})] values : [1]
  	%0 = relalg.const_relation @constrel  attributes : [@attr1({type = i32})] values : [1, 2]
  	%1 = relalg.const_relation @constrel2  attributes : [@attr1({type = i32})] values : [1]
...
module @querymodule  {
  func @query() {
	...
	//CHECK: %{{.*}} = relalg.getattr %arg0 @constrel::@attr1 : i32
	//CHECK: %{{.*}} = relalg.getattr %arg0 @markjoin::@markattr : i1
  	%2 = relalg.selection %0 (%arg0: !relalg.tuple) {
  		%3 = relalg.getattr %arg0 @constrel::@attr1 : i32
  	    %4 = relalg.in %3 : i32, %1
