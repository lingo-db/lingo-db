...
class AtLowering : public OpConversionPattern<dsa::At> {
   ...
   Value loadValue(OpBuilder rewriter, mlir::Location loc, mlir::Type baseType, mlir::Value originalValueBuffer,
      ...
      if (mlir::isa<dsa::ArrowStringType>(baseType)) {
         ...
         assert(convertedType.getIntOrFloatBitWidth() == 32 && "char from fixed size binary requires exactly 32 bits (max utf-8 char) for storage");
         assert(fixedSizeType.getByteWidth() == 4 && "fixed size binary must be 4 bytes for char");
