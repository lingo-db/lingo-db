...
class EntryStorageHelper {
   ...
   class LazyValueMap {
      ...
      public:
      LazyValueMap(mlir::Value ref, mlir::OpBuilder& rewriter, mlir::Location loc, const EntryStorageHelper& esh, ArrayAttr relevantMembers) : ref(esh.ensureRefType(ref, rewriter, loc)), rewriter(rewriter), loc(loc), esh(esh), relevantMembers(relevantMembers) {
         if (!relevantMembers) {
         ...
         for (auto& [name, value] : values) {
            ...
            if (memberInfo.isNullable) {
               ...
               if (nullBitCache.contains(name)) {
                  ...
                  const mlir::Value clearBit = rewriter.create<mlir::arith::ConstantIntOp>(loc, 1, nullBit.getType());
                  ...
                  const mlir::Value shiftedClearBit = rewriter.create<mlir::arith::ShLIOp>(loc, rewriter.create<mlir::arith::ExtUIOp>(loc, esh.nullBitsetType, clearBit), shiftAmount);
                  const mlir::Value invertedShiftedClearBit = rewriter.create<mlir::arith::XOrIOp>(loc, shiftedClearBit, rewriter.create<mlir::arith::ConstantIntOp>(loc, -1, esh.nullBitsetType));
