...
std::shared_ptr<ast::AstNode> SQLQueryAnalyzer::canonicalizeAndAnalyze(std::shared_ptr<ast::AstNode> astRootNode, std::shared_ptr<SQLContext> context) {
   ...
   if (!rootNode) {
      ...
      for (auto target : context->currentScope->targetInfo.getTargetColumns()) {
         if (!target->isSelectable) {
            error("Column " + target->displayName + " is not available after aggregation", transformed->loc);
         }
      }
      ...
      case ast::NodeType::QUERY_NODE: {
         ...
         switch (queryNode->type) {
            case ast::QueryNodeType::CTE_NODE: {
               ...
               if (cteNode->query) {
                  ast::TargetInfo targetInfo{};
                  {
                     ...
                     size_t i = 0;
                     for (auto targetColumns : targetInfo.getTargetColumns()) {
            ...
            case ast::QueryNodeType::SET_OPERATION_NODE: {
               ...
               if (leftScope->targetInfo.getTargetColumns().size() != rightScope->targetInfo.getTargetColumns().size()) {
               ...
               for (size_t i = 0; i < leftScope->targetInfo.getTargetColumns().size(); i++) {
                  auto leftColumn = leftScope->targetInfo.getTargetColumn(i);
                  auto rightColumn = rightScope->targetInfo.getTargetColumn(i);
               ...
               context->currentScope->targetInfo.setTargetColumns(newTargetInfos);
...
std::shared_ptr<ast::CreateNode> SQLQueryAnalyzer::analyzeCreateNode(std::shared_ptr<ast::CreateNode> createNode) {
   switch (createNode->createInfo->type) {
      ...
      case catalog::CatalogEntry::CatalogEntryType::C_FUNCTION_ENTRY: {
         ...
         analyzeScalarFunctionCreate(createNode, createFunctionInfo, context, resolverScope);
         return createNode;
      }
      case catalog::CatalogEntry::CatalogEntryType::TABLE_FUNCTION_ENTRY: {
         auto createTableFunctionInfo = std::static_pointer_cast<ast::CreateTableFunctionInfo>(createNode->createInfo);
         auto resolverScope = context->createResolverScope();
         analyzeTableFunctionCreate(createNode, createTableFunctionInfo, context, resolverScope);
         return createNode;
      }
...
std::shared_ptr<ast::CreateNode> SQLQueryAnalyzer::analyzeScalarFunctionCreate(std::shared_ptr<ast::CreateNode> createNode, std::shared_ptr<ast::CreateFunctionInfo> createFunctionInfo, std::shared_ptr<SQLContext> context, ResolverScope& resolverScope) {
...
std::shared_ptr<ast::CreateNode> SQLQueryAnalyzer::analyzeTableFunctionCreate(std::shared_ptr<ast::CreateNode> createNode, std::shared_ptr<ast::CreateTableFunctionInfo> createTableFunctionInfo, std::shared_ptr<SQLContext> context, ResolverScope& resolverScope) {
   std::string language = "unknown";
   std::string code = "";
   for (auto [optionName, optionValue] : createTableFunctionInfo->options) {
      if (optionName == "LANGUAGE") {
         language = optionValue;
      } else if (optionName == "AS") {
         code = optionValue;
   ...
   for (auto& [colName, colType] : createTableFunctionInfo->returnColumns) {
      bound->returnColumns.emplace_back(
         colName,
         SQLTypeUtils::typemodsToCatalogType(colType.logicalTypeId, colType.typeModifiers).type);
   }
   bound->options = createTableFunctionInfo->options;

   createNode->createInfo = bound;
   return createNode;
}
   ...
   switch (pipeOperator->pipeOpType) {
      ...
      case ast::PipeOperatorType::DROP: {
         ...
         for (auto& target : targets->targets) {
            ...
            assert(boundExpression->columnReference.has_value());
            context->currentScope->targetInfo.eraseif([&](const std::shared_ptr<ast::ColumnReference>& other) {
...
std::shared_ptr<ast::TableProducer> SQLQueryAnalyzer::analyzeTableRef(std::shared_ptr<ast::TableRef> tableRef, std::shared_ptr<SQLContext> context, ResolverScope& resolverScope) {
   ...
   switch (tableRef->type) {
      ...
      case ast::TableReferenceType::SUBQUERY: {
         ...
         size_t i = 0;
         for (auto target : targetInfo.getTargetColumns()) {
...
std::shared_ptr<ast::TableProducer> SQLQueryAnalyzer::analyzeTableFunctionRef(std::shared_ptr<ast::TableFunctionRef> tableFunctionRef, std::shared_ptr<SQLContext> context, ResolverScope& resolverScope) {
   auto entry = catalog->getTypedEntry<catalog::TableFunctionCatalogEntry>(tableFunctionRef->functionName);
   if (!entry.has_value()) {
   ...
   columnReferences.reserve(tableFunctionEntry->getReturnColumns().size());
   for (const auto& [colName, colType] : tableFunctionEntry->getReturnColumns()) {
            ...
            if (orderByElement->expression) {
               ...
               switch (boundExpression->type) {
                  ...
                  case ast::ExpressionType::VALUE_CONSTANT: {
                     ...
                     if (context->currentScope->targetInfo.size() < static_cast<size_t>(constantValue->iVal) || constantValue->iVal <= 0) {
                        error("Invalid order by element", boundConstant->loc);
                     }
                     columnReference = context->currentScope->targetInfo.getTargetColumn(constantValue->iVal - 1);
                     break;
                  }
...
std::shared_ptr<ast::BoundExpression> SQLQueryAnalyzer::analyzeExpression(std::shared_ptr<ast::ParsedExpression> rootNode, std::shared_ptr<SQLContext> context, ResolverScope& resolverScope) {
   ...
   switch (rootNode->exprClass) {
      ...
      case ast::ExpressionClass::SUBQUERY: {
         ...
         if (subqueryTargetInfo.size() != 1 && subqueryExpr->subQueryType != ast::SubqueryType::EXISTS && subqueryExpr->subQueryType != ast::SubqueryType::NOT_EXISTS) {
            error("subquery expressions must produce a single value", subqueryExpr->loc);
         }

         auto columnReference = subqueryTargetInfo.getTargetColumn(0);
         ...
         if (subqueryExpr->subQueryType == ast::SubqueryType::EXISTS || subqueryExpr->subQueryType == ast::SubqueryType::NOT_EXISTS) {
            boundSubqueryExpression->columnReferenceForSubquery = nullptr;
            subqueryScope->targetInfo.clear();
         }
...
std::shared_ptr<ast::BoundExpression> SQLQueryAnalyzer::analyzeFunctionExpression(std::shared_ptr<ast::FunctionExpression> function, std::shared_ptr<SQLContext> context, ResolverScope& resolverScope) {
   ...
   if (upperCaseFName == "DATE") {
      ...
      // LENGTH accepts any string-typed expression, not just a bare column
      // reference — e.g. LENGTH(some_udf(...)) or LENGTH(a || b).
      auto arg = analyzeExpression(function->arguments[0], context, resolverScope);
      resultType = catalog::Type::int64();
      boundFunctionExpression = drv.nf.node<ast::BoundFunctionExpression>(function->loc, function->type, resultType, function->functionName, scope, fName, function->distinct, std::vector{arg});
