...
// Push tuple-stream columns through nested map/loop bodies so combine_tuple ops do
// not cross step boundaries, then split those bodies into NestedExecutionGroupOps
// of ExecutionStepOps (required by loop / nested_map lowering).
class PrepareLoweringPass : public mlir::PassWrapper<PrepareLoweringPass, mlir::OperationPass<mlir::ModuleOp>> {
   ...
   PrepareLoweringPass(bool loopsOnly) : loopsOnly(loopsOnly) {}
   virtual llvm::StringRef getArgument() const override { return "subop-prepare-lowering"; }

   // When true only loop bodies are split (so they can be parallelized before the
   // rest of the pipeline runs); nested_map bodies are left for the later run.
   bool loopsOnly;
   ...
   void runOnOperation() override {
      if (loopsOnly) {
         getOperation()->walk([&](subop::ExecutionGroupOp eg) { splitNested(eg); });
         return;
      }
...
void PrepareLoweringPass::splitNested(subop::ExecutionGroupOp executionGroupOp) { // we take the query
   ...
   for (auto containsNestedSubOps : opsWithNesting) {
      if (loopsOnly && !mlir::isa<subop::LoopOp>(containsNestedSubOps.getOperation())) continue;
      llvm::DenseMap<mlir::Value, size_t> valueToNestedExecutionGroupOperand;

      mlir::Block* block = containsNestedSubOps.getBody();
      // Already split (e.g. loop body handled by an earlier loops-only run).
      if (mlir::isa<subop::NestedExecutionGroupOp>(block->front())) continue;
...
std::unique_ptr<mlir::Pass>
subop::createPrepareLoweringPass(bool loopsOnly) { return std::make_unique<PrepareLoweringPass>(loopsOnly); }
