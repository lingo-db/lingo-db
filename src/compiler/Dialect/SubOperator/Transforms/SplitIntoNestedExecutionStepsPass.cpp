...
// Wraps each `ContainsNestedSubOps` op's body (NestedMapOp / LoopOp) in a
// `subop.nested_execution_group` containing per-pipeline `subop.execution_step`s.
// Counterpart to `SplitIntoExecutionStepsPass`, which does the same thing one
// level up at the top-level `subop.execution_group`. Extracted from the original
// PrepareForLoweringPass so that InlineNestedMapPass can run between the two
// splitting steps and operate on bodies that aren't yet wrapped in inner
// execution_step regions.
