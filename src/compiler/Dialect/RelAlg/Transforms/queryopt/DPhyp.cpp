...
void relalg::DPHyp::emitCsgCmp(const NodeSet& s1, const NodeSet& s2) {
   auto p1 = dpTable[s1];
   auto p2 = dpTable[s2];
   NodeSet s;
...
void relalg::DPHyp::enumerateCsgRec(NodeSet s1, NodeSet x) {
...
void relalg::DPHyp::enumerateCmpRec(NodeSet s1, NodeSet s2, NodeSet x) {
...
void relalg::DPHyp::emitCsg(NodeSet s1) {
...
std::shared_ptr<relalg::Plan> relalg::DPHyp::createInitialPlan(relalg::QueryGraph::Node& n) {
...
std::shared_ptr<relalg::Plan> relalg::DPHyp::solve() {
...
void relalg::DPHyp::countSubGraphsRec(NodeSet s1, NodeSet x, size_t& count, size_t maxCount) {
   auto neighbors = queryGraph.getNeighbors(s1, x);
   neighbors.iterateSubsets([&](const NodeSet& n) {
...
size_t relalg::DPHyp::countSubGraphs(size_t maxCount) {
   size_t count = 0;
   queryGraph.iterateNodesDesc([&](QueryGraph::Node& v) {
      auto onlyV = NodeSet::single(queryGraph.numNodes, v.id);
      if ((++count) >= maxCount) return;
