...
void relalg::QueryGraphBuilder::populateQueryGraph(Operator op) {
   auto children = op.getChildren();
   auto used = op.getUsedColumns();
   auto created = op.getCreatedColumns();
   if (alreadyOptimized.count(op.getOperation())) {
   ...
   if (mlir::isa<relalg::CrossProductOp>(op.getOperation())) {
      //do not construct crossproducts in the querygraph
   } else if (mlir::isa<relalg::SelectionOp>(op.getOperation()) || mlir::isa<relalg::InnerJoinOp>(op.getOperation())) {
      NodeSet ses = calcSES(op);
      if (!ses.any() && mlir::isa<relalg::SelectionOp>(op.getOperation())) {
         ses = calcT(op);
      }
      if (ses.count() == 1) {
   ...
   } else if (relalg::detail::isJoin(op.getOperation())) {
