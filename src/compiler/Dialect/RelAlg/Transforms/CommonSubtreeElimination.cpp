...
// Set to 1 to enable statistics printing in Release mode
#define CSE_DEBUG 0
...
class CommonSubtreeElimination; // Forward declaration
...
class CommonSubtreeElimination : public mlir::PassWrapper<CommonSubtreeElimination, mlir::OperationPass<mlir::func::FuncOp>> {
   ...
   // Explicit counters for statistics (printing controlled by CSE_DEBUG)
...
bool EquivalenceChecker::checkAttributes(mlir::Operation* leader, mlir::Operation* candidate) const {
   llvm::SmallSetVector<mlir::StringAttr, 4> names;
...
bool EquivalenceChecker::checkOps(mlir::Operation* leader, mlir::Operation* candidate) {
...
bool EquivalenceChecker::checkRegions(mlir::Region& r1, mlir::Region& r2) {
...
mlir::Attribute CommonSubtreeElimination::getCanonicalAttr(mlir::Attribute attr, const llvm::DenseMap<const tuples::Column*, std::string>* localDefs) const {
   if (!attr) return {};
...
llvm::hash_code CommonSubtreeElimination::computeHash(mlir::Operation* op) const {
...
bool CommonSubtreeElimination::areEquivalent(mlir::Operation* leader, mlir::Operation* candidate) const {
...
void CommonSubtreeElimination::processBlock(
   mlir::Block* block,
   mlir::DominanceInfo& domInfo,
   llvm::DenseMap<llvm::hash_code, llvm::SmallVector<mlir::Operation*, 2>>& candidates,
   llvm::function_ref<void(mlir::Region&)>
      traverseRegion) {
...
void CommonSubtreeElimination::runOnOperation() {
...
void CommonSubtreeElimination::mapVirtualBaseTableOpMerge(mlir::Operation* leader, mlir::Operation* duplicate) {
...
void CommonSubtreeElimination::mapVirtualMerge(mlir::Operation* leader, mlir::Operation* duplicate) {
...
void CommonSubtreeElimination::resolveLeaderColumn(
...
void CommonSubtreeElimination::mergeNonBaseTableOp(mlir::Operation* leader, mlir::Operation* duplicate) {
   llvm::SmallVector<std::pair<std::string, tuples::ColumnDefAttr>> lDefs, dDefs;
   collectSortedLocalDefs(leader, lDefs);
   collectSortedLocalDefs(duplicate, dDefs);
