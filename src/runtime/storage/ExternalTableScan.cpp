...
arrow::Status ScanParquetFileTask::init(std::vector<FilterDescription>& filterDescriptions) {
   ...
   for (const auto& filter : filterDescriptions) {
      ...
      std::variant<std::string, int64_t, double, int32_t> comparableValue;
      ...
      if (typeId == arrow::Type::INT32 || typeId == arrow::Type::INT64) {
         ...
         hasComparableValue = true;
      } else if (typeId == arrow::Type::DATE32) {
         assert(std::holds_alternative<std::string>(filter.value));
         auto parsedDateValue = parseDate32(std::get<std::string>(filter.value));
         comparableValue = parsedDateValue;
         hasComparableValue = true;
      }
