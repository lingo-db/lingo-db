...
WASMSession* WASM::initializeWASM() {
   ...
   // The lingodb scheduler puts every fiber stack inside the worker's pthread
   // stack range, so WAMR detects the bounds via pthread_attr_getstack and we
   // do not need to call wasm_runtime_set_native_stack_boundary here.
