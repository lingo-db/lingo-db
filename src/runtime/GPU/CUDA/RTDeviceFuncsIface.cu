#include "lingodb/runtime/GPU/CUDA/Locking.cuh"
#include "lingodb/runtime/GPU/CUDA/GrowingBuffer.cuh"
#include "lingodb/runtime/GPU/CUDA/PreAggregationHashTable.cuh"
#include "lingodb/runtime/GPU/CUDA/helpers.cuh"

// #include "Vector.cu"

__device__ __forceinline__ int nearestPow2LessThan(const int value) {
    if (value <= 0) return 0;
    return 1 << (31 - __clz(value));  
}

template<int N>
struct NullableI32{
    bool null;
    int arr[N];
};

extern "C" {
__device__ void _mlir_ciface_acquireLock(int* lock) {
    acquireLock(lock);
}
__device__ void _mlir_ciface_relLock(int* lock){
    relLock(lock);
}
__device__ void _mlir_ciface_acquireLockBlock(int* lock){
    acquireLockBlock(lock);
}
__device__ void _mlir_ciface_relLockBlock(int* lock) {
    relLockBlock(lock);
}

__device__ void _mlir_ciface_syncWarp() {
    __syncwarp();
}

__device__ uint8_t* _mlir_ciface_shfl_down_sync(uint8_t* statePtr, int offset){
    return (uint8_t*)__shfl_down_sync(0xFFFFFFFF, (unsigned long long)statePtr, offset);
}


__device__ void _mlir_ciface_printfNullableI32(uint8_t* ptr){
    NullableI32<1>* typedPtr = reinterpret_cast<NullableI32<1>*>(&ptr[1]);
    printf("printfNullableI32[%p] : %d, %d\n", typedPtr, typedPtr->null, typedPtr->arr[0]);
}
__device__ void _mlir_ciface_printfNullable2xI32(uint8_t* ptr){
    NullableI32<2>* typedPtr = reinterpret_cast<NullableI32<2>*>(&ptr[1]);
    printf("printfNullableI32[%p] : %d, %d, %d\n", typedPtr, typedPtr->null, typedPtr->arr[0], typedPtr->arr[1]);
}
__device__ void _mlir_ciface_printfNullable3xI32(uint8_t* ptr){
    NullableI32<3>* typedPtr = reinterpret_cast<NullableI32<3>*>(ptr);
    printf("printfNullableI32[%p] :  %d, %d, %d\n", typedPtr, typedPtr->arr[0], typedPtr->arr[1], typedPtr->arr[2]);
}
__device__ void _mlir_ciface_printfI32(uint8_t* ptr){
    int* typedPtr = reinterpret_cast<int*>(ptr);
    printf("printfI32[%p] : %d\n", typedPtr, typedPtr[0]);
}
__device__ void _mlir_ciface_printf2xI32(uint8_t* ptr){
    int* typedPtr = reinterpret_cast<int*>(ptr);
    printf("printf2xI32[%p] : %d, %d\n", typedPtr, typedPtr[0], typedPtr[1]);
}
__constant__ uint16_t bloomMasks[2048] = {
   // The 1820 distinct bit-patterns
      15, 23, 27, 29, 30, 39, 43, 45, 46, 51, 53, 54, 57, 58, 60, 71, 75, 77, 78, 83, 85, 86, 89, 90, 92, 99, 101, 102, 105, 106, 108, 113, 114, 116, 120, 135, 139, 141, 142, 147, 149, 150, 153, 154, 156, 163, 165, 166, 169, 170, 172, 177, 178, 180, 184, 195, 197, 198, 201, 202, 204, 209, 210, 212, 216, 225, 226, 228, 232, 240, 263, 267, 269, 270, 275, 277, 278, 281, 282, 284, 291, 293, 294, 297, 298, 300, 305, 306, 308, 312, 323, 325, 326, 329, 330, 332, 337, 338, 340, 344, 353, 354, 356, 360, 368, 387, 389, 390, 393, 394, 396, 401, 402, 404, 408, 417, 418, 420, 424, 432, 449, 450, 452, 456, 464, 480, 519, 523,
      525, 526, 531, 533, 534, 537, 538, 540, 547, 549, 550, 553, 554, 556, 561, 562, 564, 568, 579, 581, 582, 585, 586, 588, 593, 594, 596, 600, 609, 610, 612, 616, 624, 643, 645, 646, 649, 650, 652, 657, 658, 660, 664, 673, 674, 676, 680, 688, 705, 706, 708, 712, 720, 736, 771, 773, 774, 777, 778, 780, 785, 786, 788, 792, 801, 802, 804, 808, 816, 833, 834, 836, 840, 848, 864, 897, 898, 900, 904, 912, 928, 960, 1031, 1035, 1037, 1038, 1043, 1045, 1046, 1049, 1050, 1052, 1059, 1061, 1062, 1065, 1066, 1068, 1073, 1074, 1076, 1080, 1091, 1093, 1094, 1097, 1098, 1100, 1105, 1106, 1108, 1112, 1121, 1122, 1124, 1128, 1136, 1155, 1157, 1158, 1161, 1162, 1164, 1169, 1170, 1172, 1176, 1185,
      1186, 1188, 1192, 1200, 1217, 1218, 1220, 1224, 1232, 1248, 1283, 1285, 1286, 1289, 1290, 1292, 1297, 1298, 1300, 1304, 1313, 1314, 1316, 1320, 1328, 1345, 1346, 1348, 1352, 1360, 1376, 1409, 1410, 1412, 1416, 1424, 1440, 1472, 1539, 1541, 1542, 1545, 1546, 1548, 1553, 1554, 1556, 1560, 1569, 1570, 1572, 1576, 1584, 1601, 1602, 1604, 1608, 1616, 1632, 1665, 1666, 1668, 1672, 1680, 1696, 1728, 1793, 1794, 1796, 1800, 1808, 1824, 1856, 1920, 2055, 2059, 2061, 2062, 2067, 2069, 2070, 2073, 2074, 2076, 2083, 2085, 2086, 2089, 2090, 2092, 2097, 2098, 2100, 2104, 2115, 2117, 2118, 2121, 2122, 2124, 2129, 2130, 2132, 2136, 2145, 2146, 2148, 2152, 2160, 2179, 2181, 2182, 2185, 2186, 2188, 2193, 2194, 2196, 2200, 2209, 2210, 2212, 2216, 2224, 2241, 2242, 2244, 2248,
      2256, 2272, 2307, 2309, 2310, 2313, 2314, 2316, 2321, 2322, 2324, 2328, 2337, 2338, 2340, 2344, 2352, 2369, 2370, 2372, 2376, 2384, 2400, 2433, 2434, 2436, 2440, 2448, 2464, 2496, 2563, 2565, 2566, 2569, 2570, 2572, 2577, 2578, 2580, 2584, 2593, 2594, 2596, 2600, 2608, 2625, 2626, 2628, 2632, 2640, 2656, 2689, 2690, 2692, 2696, 2704, 2720, 2752, 2817, 2818, 2820, 2824, 2832, 2848, 2880, 2944, 3075, 3077, 3078, 3081, 3082, 3084, 3089, 3090, 3092, 3096, 3105, 3106, 3108, 3112, 3120, 3137, 3138, 3140, 3144, 3152, 3168, 3201, 3202, 3204, 3208, 3216, 3232, 3264, 3329, 3330, 3332, 3336, 3344, 3360, 3392, 3456, 3585, 3586, 3588, 3592, 3600, 3616, 3648, 3712, 3840, 4103, 4107, 4109, 4110, 4115, 4117, 4118, 4121, 4122, 4124, 4131, 4133, 4134, 4137, 4138, 4140, 4145,
      4146, 4148, 4152, 4163, 4165, 4166, 4169, 4170, 4172, 4177, 4178, 4180, 4184, 4193, 4194, 4196, 4200, 4208, 4227, 4229, 4230, 4233, 4234, 4236, 4241, 4242, 4244, 4248, 4257, 4258, 4260, 4264, 4272, 4289, 4290, 4292, 4296, 4304, 4320, 4355, 4357, 4358, 4361, 4362, 4364, 4369, 4370, 4372, 4376, 4385, 4386, 4388, 4392, 4400, 4417, 4418, 4420, 4424, 4432, 4448, 4481, 4482, 4484, 4488, 4496, 4512, 4544, 4611, 4613, 4614, 4617, 4618, 4620, 4625, 4626, 4628, 4632, 4641, 4642, 4644, 4648, 4656, 4673, 4674, 4676, 4680, 4688, 4704, 4737, 4738, 4740, 4744, 4752, 4768, 4800, 4865, 4866, 4868, 4872, 4880, 4896, 4928, 4992, 5123, 5125, 5126, 5129, 5130, 5132, 5137, 5138, 5140, 5144, 5153, 5154, 5156, 5160, 5168, 5185, 5186, 5188, 5192, 5200, 5216, 5249, 5250, 5252, 5256,
      5264, 5280, 5312, 5377, 5378, 5380, 5384, 5392, 5408, 5440, 5504, 5633, 5634, 5636, 5640, 5648, 5664, 5696, 5760, 5888, 6147, 6149, 6150, 6153, 6154, 6156, 6161, 6162, 6164, 6168, 6177, 6178, 6180, 6184, 6192, 6209, 6210, 6212, 6216, 6224, 6240, 6273, 6274, 6276, 6280, 6288, 6304, 6336, 6401, 6402, 6404, 6408, 6416, 6432, 6464, 6528, 6657, 6658, 6660, 6664, 6672, 6688, 6720, 6784, 6912, 7169, 7170, 7172, 7176, 7184, 7200, 7232, 7296, 7424, 7680, 8199, 8203, 8205, 8206, 8211, 8213, 8214, 8217, 8218, 8220, 8227, 8229, 8230, 8233, 8234, 8236, 8241, 8242, 8244, 8248, 8259, 8261, 8262, 8265, 8266, 8268, 8273, 8274, 8276, 8280, 8289, 8290, 8292, 8296, 8304, 8323, 8325, 8326, 8329, 8330, 8332, 8337, 8338, 8340, 8344, 8353, 8354, 8356, 8360, 8368, 8385, 8386, 8388,
      8392, 8400, 8416, 8451, 8453, 8454, 8457, 8458, 8460, 8465, 8466, 8468, 8472, 8481, 8482, 8484, 8488, 8496, 8513, 8514, 8516, 8520, 8528, 8544, 8577, 8578, 8580, 8584, 8592, 8608, 8640, 8707, 8709, 8710, 8713, 8714, 8716, 8721, 8722, 8724, 8728, 8737, 8738, 8740, 8744, 8752, 8769, 8770, 8772, 8776, 8784, 8800, 8833, 8834, 8836, 8840, 8848, 8864, 8896, 8961, 8962, 8964, 8968, 8976, 8992, 9024, 9088, 9219, 9221, 9222, 9225, 9226, 9228, 9233, 9234, 9236, 9240, 9249, 9250, 9252, 9256, 9264, 9281, 9282, 9284, 9288, 9296, 9312, 9345, 9346, 9348, 9352, 9360, 9376, 9408, 9473, 9474, 9476, 9480, 9488, 9504, 9536, 9600, 9729, 9730, 9732, 9736, 9744, 9760, 9792, 9856, 9984, 10243, 10245, 10246, 10249, 10250, 10252, 10257, 10258, 10260, 10264, 10273, 10274, 10276, 10280, 10288, 10305,
      10306, 10308, 10312, 10320, 10336, 10369, 10370, 10372, 10376, 10384, 10400, 10432, 10497, 10498, 10500, 10504, 10512, 10528, 10560, 10624, 10753, 10754, 10756, 10760, 10768, 10784, 10816, 10880, 11008, 11265, 11266, 11268, 11272, 11280, 11296, 11328, 11392, 11520, 11776, 12291, 12293, 12294, 12297, 12298, 12300, 12305, 12306, 12308, 12312, 12321, 12322, 12324, 12328, 12336, 12353, 12354, 12356, 12360, 12368, 12384, 12417, 12418, 12420, 12424, 12432, 12448, 12480, 12545, 12546, 12548, 12552, 12560, 12576, 12608, 12672, 12801, 12802, 12804, 12808, 12816, 12832, 12864, 12928, 13056, 13313, 13314, 13316, 13320, 13328, 13344, 13376, 13440, 13568, 13824, 14337, 14338, 14340, 14344, 14352, 14368, 14400, 14464, 14592, 14848, 15360, 16391, 16395, 16397, 16398, 16403, 16405, 16406, 16409, 16410, 16412, 16419, 16421, 16422, 16425, 16426, 16428, 16433, 16434, 16436, 16440, 16451, 16453, 16454,
      16457, 16458, 16460, 16465, 16466, 16468, 16472, 16481, 16482, 16484, 16488, 16496, 16515, 16517, 16518, 16521, 16522, 16524, 16529, 16530, 16532, 16536, 16545, 16546, 16548, 16552, 16560, 16577, 16578, 16580, 16584, 16592, 16608, 16643, 16645, 16646, 16649, 16650, 16652, 16657, 16658, 16660, 16664, 16673, 16674, 16676, 16680, 16688, 16705, 16706, 16708, 16712, 16720, 16736, 16769, 16770, 16772, 16776, 16784, 16800, 16832, 16899, 16901, 16902, 16905, 16906, 16908, 16913, 16914, 16916, 16920, 16929, 16930, 16932, 16936, 16944, 16961, 16962, 16964, 16968, 16976, 16992, 17025, 17026, 17028, 17032, 17040, 17056, 17088, 17153, 17154, 17156, 17160, 17168, 17184, 17216, 17280, 17411, 17413, 17414, 17417, 17418, 17420, 17425, 17426, 17428, 17432, 17441, 17442, 17444, 17448, 17456, 17473, 17474, 17476, 17480, 17488, 17504, 17537, 17538, 17540, 17544, 17552, 17568, 17600, 17665, 17666, 17668,
   17672, 17680, 17696, 17728, 17792, 17921, 17922, 17924, 17928, 17936, 17952, 17984, 18048, 18176, 18435, 18437, 18438, 18441, 18442, 18444, 18449, 18450, 18452, 18456, 18465, 18466, 18468, 18472, 18480, 18497, 18498, 18500, 18504, 18512, 18528, 18561, 18562, 18564, 18568, 18576, 18592, 18624, 18689, 18690, 18692, 18696, 18704, 18720, 18752, 18816, 18945, 18946, 18948, 18952, 18960, 18976, 19008, 19072, 19200, 19457, 19458, 19460, 19464, 19472, 19488, 19520, 19584, 19712, 19968, 20483, 20485, 20486, 20489, 20490, 20492, 20497, 20498, 20500, 20504, 20513, 20514, 20516, 20520, 20528, 20545, 20546, 20548, 20552, 20560, 20576, 20609, 20610, 20612, 20616, 20624, 20640, 20672, 20737, 20738, 20740, 20744, 20752, 20768, 20800, 20864, 20993, 20994, 20996, 21000, 21008, 21024, 21056, 21120, 21248, 21505, 21506, 21508, 21512, 21520, 21536, 21568, 21632, 21760, 22016, 22529, 22530, 22532, 22536,
      22544, 22560, 22592, 22656, 22784, 23040, 23552, 24579, 24581, 24582, 24585, 24586, 24588, 24593, 24594, 24596, 24600, 24609, 24610, 24612, 24616, 24624, 24641, 24642, 24644, 24648, 24656, 24672, 24705, 24706, 24708, 24712, 24720, 24736, 24768, 24833, 24834, 24836, 24840, 24848, 24864, 24896, 24960, 25089, 25090, 25092, 25096, 25104, 25120, 25152, 25216, 25344, 25601, 25602, 25604, 25608, 25616, 25632, 25664, 25728, 25856, 26112, 26625, 26626, 26628, 26632, 26640, 26656, 26688, 26752, 26880, 27136, 27648, 28673, 28674, 28676, 28680, 28688, 28704, 28736, 28800, 28928, 29184, 29696, 30720, 32775, 32779, 32781, 32782, 32787, 32789, 32790, 32793, 32794, 32796, 32803, 32805, 32806, 32809, 32810, 32812, 32817, 32818, 32820, 32824, 32835, 32837, 32838, 32841, 32842, 32844, 32849, 32850, 32852, 32856, 32865, 32866, 32868, 32872, 32880, 32899, 32901, 32902, 32905, 32906, 32908, 32913, 32914,
      32916, 32920, 32929, 32930, 32932, 32936, 32944, 32961, 32962, 32964, 32968, 32976, 32992, 33027, 33029, 33030, 33033, 33034, 33036, 33041, 33042, 33044, 33048, 33057, 33058, 33060, 33064, 33072, 33089, 33090, 33092, 33096, 33104, 33120, 33153, 33154, 33156, 33160, 33168, 33184, 33216, 33283, 33285, 33286, 33289, 33290, 33292, 33297, 33298, 33300, 33304, 33313, 33314, 33316, 33320, 33328, 33345, 33346, 33348, 33352, 33360, 33376, 33409, 33410, 33412, 33416, 33424, 33440, 33472, 33537, 33538, 33540, 33544, 33552, 33568, 33600, 33664, 33795, 33797, 33798, 33801, 33802, 33804, 33809, 33810, 33812, 33816, 33825, 33826, 33828, 33832, 33840, 33857, 33858, 33860, 33864, 33872, 33888, 33921, 33922, 33924, 33928, 33936, 33952, 33984, 34049, 34050, 34052, 34056, 34064, 34080, 34112, 34176, 34305, 34306, 34308, 34312, 34320, 34336, 34368, 34432, 34560, 34819, 34821, 34822, 34825, 34826, 34828,
      34833, 34834, 34836, 34840, 34849, 34850, 34852, 34856, 34864, 34881, 34882, 34884, 34888, 34896, 34912, 34945, 34946, 34948, 34952, 34960, 34976, 35008, 35073, 35074, 35076, 35080, 35088, 35104, 35136, 35200, 35329, 35330, 35332, 35336, 35344, 35360, 35392, 35456, 35584, 35841, 35842, 35844, 35848, 35856, 35872, 35904, 35968, 36096, 36352, 36867, 36869, 36870, 36873, 36874, 36876, 36881, 36882, 36884, 36888, 36897, 36898, 36900, 36904, 36912, 36929, 36930, 36932, 36936, 36944, 36960, 36993, 36994, 36996, 37000, 37008, 37024, 37056, 37121, 37122, 37124, 37128, 37136, 37152, 37184, 37248, 37377, 37378, 37380, 37384, 37392, 37408, 37440, 37504, 37632, 37889, 37890, 37892, 37896, 37904, 37920, 37952, 38016, 38144, 38400, 38913, 38914, 38916, 38920, 38928, 38944, 38976, 39040, 39168, 39424, 39936, 40963, 40965, 40966, 40969, 40970, 40972, 40977, 40978, 40980, 40984, 40993, 40994, 40996,
      41000, 41008, 41025, 41026, 41028, 41032, 41040, 41056, 41089, 41090, 41092, 41096, 41104, 41120, 41152, 41217, 41218, 41220, 41224, 41232, 41248, 41280, 41344, 41473, 41474, 41476, 41480, 41488, 41504, 41536, 41600, 41728, 41985, 41986, 41988, 41992, 42000, 42016, 42048, 42112, 42240, 42496, 43009, 43010, 43012, 43016, 43024, 43040, 43072, 43136, 43264, 43520, 44032, 45057, 45058, 45060, 45064, 45072, 45088, 45120, 45184, 45312, 45568, 46080, 47104, 49155, 49157, 49158, 49161, 49162, 49164, 49169, 49170, 49172, 49176, 49185, 49186, 49188, 49192, 49200, 49217, 49218, 49220, 49224, 49232, 49248, 49281, 49282, 49284, 49288, 49296, 49312, 49344, 49409, 49410, 49412, 49416, 49424, 49440, 49472, 49536, 49665, 49666, 49668, 49672, 49680, 49696, 49728, 49792, 49920, 50177, 50178, 50180, 50184, 50192, 50208, 50240, 50304, 50432, 50688, 51201, 51202, 51204, 51208, 51216, 51232, 51264, 51328,
      51456, 51712, 52224, 53249, 53250, 53252, 53256, 53264, 53280, 53312, 53376, 53504, 53760, 54272, 55296, 57345, 57346, 57348, 57352, 57360, 57376, 57408, 57472, 57600, 57856, 58368, 59392, 61440,
   // 228 repeated bit-patterns, randomly sampled from above
      75, 83, 90, 99, 116, 135, 139, 172, 284, 298, 464, 547, 556, 564, 582, 594, 657, 658, 705, 771, 780, 928, 960, 1045, 1098, 1158, 1176, 1185, 1186, 1283, 1346, 1424, 1576, 2059, 2067, 2083, 2085, 2089, 2115, 2181, 2186, 2188, 2194, 2310, 2369, 2372, 2632, 3105, 3106, 3152, 3392, 3840, 4121, 4138, 4140, 4152, 4208, 4234, 4241, 4362, 4364, 4418, 4488, 4613, 4617, 4674, 4744, 4752, 4992, 5123, 5126, 5132, 5138, 5140, 5256, 6210, 6276, 7176, 8214, 8218, 8233, 8248, 8329, 8337, 8340, 8344, 8360, 8386, 8392, 8416, 8457, 8472, 8488, 8592, 9352, 9476, 9600, 9736, 9744, 9792, 10336, 10376, 10500, 10512, 10754, 11266, 12291, 12300, 12306, 12308, 12322, 12324, 12420, 12432, 12560, 13314, 13316, 13440, 13824, 16397, 16453, 16529, 16580, 16688, 16906, 16929, 16962, 16968,
      17040, 17160, 17411, 17414, 17428, 17448, 17473, 17480, 17504, 17538, 17544, 17665, 17680, 17928, 17984, 18512, 18696, 18945, 19008, 19464, 19488, 20497, 20500, 20513, 20546, 20548, 20612, 20624, 22530, 24593, 24612, 24644, 24706, 25089, 25092, 25096, 25104, 32779, 32789, 32794, 32824, 32850, 32905, 32914, 32929, 32962, 32976, 33060, 33410, 33424, 33664, 33825, 33921, 33922, 33936, 34052, 34080, 34308, 34822, 34828, 34856, 34881, 34884, 34945, 36096, 36884, 36898, 36932, 37056, 37184, 37377, 38016, 38914, 38928, 41120, 41152, 41600, 42240, 43520, 45312, 49155, 49157, 49169, 49172, 49186, 49296, 49410, 49536, 50178, 50184, 50192, 52224, 53280, 54272, 55296, 57352, 57360, 57600, 57856, 59392
   };
__device__ uint16_t* _mlir_ciface_getBloomMasksPtr(uint8_t* ptr) {
 // printf("getBloomMasksPtr[%p]\n", ptr);
 return bloomMasks;
 }

/*
    We implement hierarchical merging thread->warp->thread_block.
*/
typedef void (*MergeFn)(void*, void*);
__device__ void _mlir_ciface_mergeThreadToWarp(uint8_t* myThreadLocalState, MergeFn mergeFn, const uint32_t typeSize){
    const int warpLaneIdx = threadIdx.x % warpSize;
    __syncwarp(); // all threads of a warp must be have their results ready
    for (int offset = warpSize / 2; offset > 0; offset /= 2) { // tree-merge
        uint8_t* otherInstance = _mlir_ciface_shfl_down_sync(myThreadLocalState, offset); // lock step via _sync
        if (warpLaneIdx < offset && myThreadLocalState && otherInstance) {
            mergeFn(myThreadLocalState, otherInstance);
        }
    }
    __syncwarp(); // warp result ready
};

// Here we use one thread in a warp for simplicity, 
// although we could use one entire warp for efficiency (but more complexity + assumption maxNumWarps = warpSize). 
__device__ __forceinline__ void mergeWarpLocal(uint8_t* myWarpLocalState, MergeFn mergeFn, const int multiplier, const uint32_t typeSize) {
    const int numWarps = blockDim.x / warpSize;
    const int warpIdxLocal = threadIdx.x / warpSize;
    const int warpLaneIdx = threadIdx.x % warpSize;
    const int pow2 = nearestPow2LessThan(numWarps);
    __syncthreads(); // all warps must have their results ready
    for (int offset = numWarps-1; offset > pow2; offset--) { // if uneven, reduce until pow2
        if (warpLaneIdx == 0 && warpIdxLocal == 0) {
            mergeFn(myWarpLocalState, &myWarpLocalState[offset * multiplier * typeSize]);
        }
    }
    __syncthreads(); // tree-merge
    for (int offset = pow2/2; offset > 0; offset /= 2) {
        if (warpLaneIdx == 0 && warpIdxLocal < offset) {
            mergeFn(myWarpLocalState, &myWarpLocalState[offset * multiplier * typeSize]);
        }
        __syncthreads(); // lock step for all warps
    }
}

__device__ void _mlir_ciface_mergeWarpFromThreadLevelToThreadBlock(uint8_t* myWarpLocalState, MergeFn mergeFn, const uint32_t typeSize) {
    mergeWarpLocal(myWarpLocalState, mergeFn, warpSize, typeSize);
}

__device__ void _mlir_ciface_mergeWarpToThreadBlock(uint8_t* myWarpLocalState, MergeFn mergeFn, const uint32_t typeSize) {
    mergeWarpLocal(myWarpLocalState, mergeFn, 1, typeSize);
}

__device__ void _mlir_ciface_mergeThreadBlockToGlobal(uint8_t* globalState, uint8_t* myThreadBlockLocalState, int* lock, MergeFn mergeFn) {
    __syncthreads(); // a thread block must have its result ready
    if (threadIdx.x == 0) {
        // printf("[GPU][_mlir_ciface_mergeThreadBlockToGlobal][BID=%d] globalState=%p, myThreadBlockLocalState=%p, lock=%p \n", blockIdx.x, globalState, myThreadBlockLocalState, lock);
        _mlir_ciface_acquireLock(lock);
        mergeFn(globalState, myThreadBlockLocalState);
        _mlir_ciface_relLock(lock);
    }
}

__device__ void _mlir_ciface_mergeThreadToGlobal(uint8_t* globalState, uint8_t* myLocalState, int* lock, MergeFn mergeFn, const uint32_t typeSize) {
    _mlir_ciface_mergeThreadToWarp(myLocalState, mergeFn, typeSize);
    _mlir_ciface_mergeWarpFromThreadLevelToThreadBlock(myLocalState, mergeFn, typeSize);
    _mlir_ciface_mergeThreadBlockToGlobal(globalState, myLocalState, lock, mergeFn);
}

__device__ void _mlir_ciface_mergeWarpToGlobal(uint8_t* globalState, uint8_t* myLocalState, int* lock, MergeFn mergeFn, const uint32_t typeSize) {
    _mlir_ciface_mergeWarpToThreadBlock(myLocalState, mergeFn, typeSize);
    _mlir_ciface_mergeThreadBlockToGlobal(globalState, myLocalState, lock, mergeFn);
}

__device__ void _mlir_ciface_GrowingBufferConstruct(cudaRT::GrowingBuffer* growingBuffer, int32_t initCapacity, int32_t typeSize) {
    // printf("[GPU][_mlir_ciface_GrowingBufferConstruct, addr=%p][LEADER BID=%d, TID=%d] type size = %d!\n", growingBuffer, blockIdx.x, threadIdx.x, typeSize);
    new(growingBuffer) cudaRT::GrowingBuffer(initCapacity, typeSize, false);
}

__device__ uint8_t* _mlir_ciface_GrowingBufferInsertWarp(cudaRT::GrowingBuffer* growingBuffer) {
    return growingBuffer->insertWarpOpportunistic();
}

__device__ void _mlir_ciface_GrowingBufferMergeIntoLeft(cudaRT::GrowingBuffer* lhs, cudaRT::GrowingBuffer* rhs) {
    // printf("[GPU][_mlir_ciface_GrowingBufferMergeIntoLeft][BID=%d] lhs=%p, rhs=%p\n", blockIdx.x, lhs, rhs);
    lhs->merge(rhs);
}


__device__ void _mlir_ciface_PreAggregationHTFragConstruct(cudaRT::PreAggregationHashtableFragment* preAggrFrag, int32_t typeSize, uint8_t* cachePtr, int32_t cacheSize) {
    new(preAggrFrag) cudaRT::PreAggregationHashtableFragment(typeSize, cachePtr, cacheSize);
}

__device__ uint8_t* _mlir_ciface_PreAggregationHTFragInsertWarp(cudaRT::PreAggregationHashtableFragment* preAggrFrag, int64_t hash) {
    return reinterpret_cast<uint8_t*>(preAggrFrag->insertWarpOpportunistic(hash));
}

// __device__ uint8_t* _mlir_ciface_PreAggregationHTFragGetCachePtr(cudaRT::PreAggregationHashtableFragment* preAggrFrag) {
//     return reinterpret_cast<uint8_t*>(preAggrFrag->getCachePtr());
// }

__device__ int64_t _mlir_ciface_PreAggregationHTFragGetCacheMask(cudaRT::PreAggregationHashtableFragment* preAggrFrag) {
    return preAggrFrag->getCacheMask();
}


__device__ void _mlir_ciface_PreAggregationHTFragMergeIntoLeft(cudaRT::PreAggregationHashtableFragment* lhs, cudaRT::PreAggregationHashtableFragment* rhs) {
    lhs->append(rhs);
}

__device__ unsigned long long _mlir_ciface_AtomicExchBlock8B(unsigned long long* ptr, unsigned long long val) {
    return atomicExch_block(ptr, val);
}

__device__ uint8_t* _mlir_ciface_getSmemPtr() {
    extern __shared__ uint8_t sharedMemory[];
    return sharedMemory;
}


__device__ uint8_t* _mlir_ciface_lockPreAggrHTEntry(uint8_t* entryPtrAddress) {
    unsigned long long next;
    do{
        next = atomicExch((unsigned long long*)entryPtrAddress, 1ull); // not null, but no entry will be at 0x1 anyways
    } while((unsigned long long)next == 1ull);
    return reinterpret_cast<uint8_t*>(next);
}

__device__ void _mlir_ciface_unlockPreAggrHTEntry(uint8_t* entryPtrAddress, uint8_t* entryPtrValue) {
    atomicExch((unsigned long long*)entryPtrAddress, (unsigned long long)entryPtrValue);
}


// This one is tricky as it consumes custom device functions and hence cannot be implemented as a plain C++ runtime call that invokes CUDA kernel (host doesn't know device function addresses).
__device__ void _mlir_ciface_fragToHT(cudaRT::PreAggregationHashtable* preAggrHT, cudaRT::PreAggregationHashtableFragment* fragment, bool (*eq)(uint8_t*, uint8_t*), void (*combine)(uint8_t*, uint8_t*)) {
    // SSB Q3.1 is an example that stresses this function.
    // Improvements ideas: hierarchical aggregation register->SMEM->Global to *reduce stress* on global locking. 
    constexpr int numElems{1024};
    __shared__ cudaRT::PreAggregationHashtable::Entry* localCache[numElems]; // optimistic - use SMEM to "fix-cache" certain entries, others get directed to global
    for(int i = threadIdx.x; i < numElems; i+=blockDim.x){
        localCache[i] = nullptr;
    }
    __syncthreads();
    cudaRT::PreAggregationHashtable::Entry* prev{nullptr}; // super optimistic - use registers to cache info on the "running" partial aggregate
    uint64_t prevHash{0};
    for(int partitionId = blockIdx.x; partitionId < cudaRT::PreAggregationHashtableFragment::numPartitions; partitionId+=gridDim.x){
        cudaRT::PreAggregationHashtable::Entry** ht = preAggrHT->ht[partitionId].ht;
        const uint64_t htMask = preAggrHT->ht[partitionId].hashMask;
        cudaRT::FlexibleBuffer* fragmentPartitionBuffer = fragment->getPartitionPtr(partitionId);
        // printf(" [TID %d] partitionId=%d, ht=%p, htMask=%lu, fragmentPartitionBuffer=%p\n", threadIdx.x, partitionId, ht, htMask, fragmentPartitionBuffer);
        cudaRT::FlexibleBufferIterator myIterator(fragmentPartitionBuffer, threadIdx.x, blockDim.x);
        cudaRT::PreAggregationHashtable::Entry* candidate = (cudaRT::PreAggregationHashtable::Entry*)myIterator.initialize();
        auto dumpEntryToGlobalHT = [&](cudaRT::PreAggregationHashtable::Entry* candidate, uint64_t candidateHash){
            const uint64_t pos = candidateHash >> cudaRT::PreAggregationHashtableFragment::partitionShift & htMask;
            cudaRT::PreAggregationHashtable::Entry* currHead;
            do { // Take pos ownership: currHead is 0 -> empty pos, currHead is 1 -> somebody works on it, else -> valid pos.
                currHead = reinterpret_cast<cudaRT::PreAggregationHashtable::Entry*>(atomicExch((unsigned long long*)&ht[pos], 1ull));
            } while((unsigned long long)currHead == 1ull);
            cudaRT::PreAggregationHashtable::Entry* currListNode = untag(currHead);
            bool merged = false;
            while (currListNode){
                // if full match -> merge partial aggregates
                if (currListNode->hashValue == candidateHash && eq((uint8_t*)currListNode->content, (uint8_t*)candidate->content)) {  
                    combine((uint8_t*)currListNode->content, (uint8_t*)candidate->content);
                    atomicAdd(&preAggrHT->numMerges, 1);
                    merged = true;
                    break;
                }
                currListNode = currListNode->next; // otherwise go to next
            }
            if (!merged) { // if not merged, then it is a new aggregate, add it to pos by updating head
                auto* taggedCandidate = tag(candidate, currHead, candidateHash); // Aggregate upper 16 bits of the bloom filter
                candidate->next = untag(currHead); // candidate is the new head, CAN'T ACCESS TAGGED DATA!
                currHead = taggedCandidate; 
                atomicAdd(&preAggrHT->size, 1);
            }
            atomicExch((unsigned long long*)&ht[pos], (unsigned long long)currHead); // release pos ownership by returning the head we have taken earlier
        };

        auto dumpEntryToLocalHT = [&](cudaRT::PreAggregationHashtable::Entry* candidate, uint64_t candidateHash){
            const uint64_t pos = candidateHash >> cudaRT::PreAggregationHashtableFragment::partitionShift & (numElems-1);
            int probeThreshold = (numElems > 64) ? (numElems / 64) : 1;
            for (int i = 0; i < probeThreshold; ++i) {
                int index = (pos + i) % numElems;
                auto beforeRead = reinterpret_cast<cudaRT::PreAggregationHashtable::Entry*>(atomicCAS_block((unsigned long long*)&localCache[index], 0, (unsigned long long)candidate));
                if(beforeRead){ // There was a valid entry!
                    do { // Take pos ownership: currHead is 0 -> empty pos, currHead is 1 -> somebody works on it, else -> valid pos.
                        beforeRead = reinterpret_cast<cudaRT::PreAggregationHashtable::Entry*>(atomicExch_block((unsigned long long*)&localCache[index], 1ull));
                    } while((unsigned long long)beforeRead == 1ull);
                    if(beforeRead->hashValue == candidateHash && eq((uint8_t*)beforeRead->content, (uint8_t*)candidate->content)) {  
                        combine((uint8_t*)beforeRead->content, (uint8_t*)candidate->content);
                        atomicExch_block((unsigned long long*)&localCache[index], (unsigned long long)beforeRead);
                        return;  // there was an entry and we have reduced it
                    } 
                    atomicExch_block((unsigned long long*)&localCache[index], (unsigned long long)beforeRead);
                } else {
                    return; // there was no entry and we have inserted a new one
                }
            }
            dumpEntryToGlobalHT(candidate, candidateHash); // no entry found, just go to global
        };

        while(candidate){
            const uint64_t candidateHash = candidate->hashValue;
            if(prev == nullptr){
                prev = candidate;
                prevHash = candidateHash;
            } else {
                if (prevHash == candidateHash && eq((uint8_t*)prev->content, (uint8_t*)candidate->content)) {  
                    combine((uint8_t*)prev->content, (uint8_t*)candidate->content);
                } else { // expensive, dump to SMEM
                    dumpEntryToLocalHT(prev, prevHash);
                    prev = candidate;
                    prevHash = candidateHash;
                }
            }
            candidate = (cudaRT::PreAggregationHashtable::Entry*)myIterator.step();
        }
        if(prev){
            dumpEntryToLocalHT(prev, prevHash);
        }
        __syncthreads();
        for(int i = threadIdx.x; i < numElems; i+=blockDim.x){
            if(localCache[i]){
                dumpEntryToGlobalHT(localCache[i], localCache[i]->hashValue);
            }
        }
    }
    // printf("[TID %d][_mlir_ciface_fragToHT] DONE!!\n", threadIdx.x);
    // preAggrHT->printMetaInfo();
}

}
