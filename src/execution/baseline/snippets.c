...
#define FOP_ORD(ty, ty2, name, op) \
   uint32_t arith_cmp_##ty2##_##name(ty a, ty b) { return !__builtin_isunordered(a, b) && (a op b); };
#define FOP_UNRD(ty, ty2, name, op) \
   uint32_t arith_cmp_##ty2##_##name(ty a, ty b) { return __builtin_isunordered(a, b) || (a op b); };
#define FOPS(ty, ty2)                                                                  \
   FOP_ORD(ty, ty2, oeq, ==)                                                           \
   FOP_ORD(ty, ty2, ogt, >)                                                            \
...
bool is_bit_set_8(uint8_t byte, uint32_t bitPos) { return (byte & (1 << bitPos)) != 0; }
bool is_bit_set_16(uint16_t word, uint32_t bitPos) { return (word & (1 << bitPos)) != 0; }
bool is_bit_set_32(uint32_t dword, uint32_t bitPos) { return (dword & (1 << bitPos)) != 0; }
bool is_bit_set_64(uint64_t qword, uint32_t bitPos) { return (qword & (1ull << bitPos)) != 0; }

// no branches
int8_t set_bit_8(int8_t byte, uint8_t bit, uint32_t bitPos) {
   bit = bit & 1;
   int8_t mask = 1 << bitPos;
   return (byte & ~mask) | (((uint8_t)bit) << bitPos);
}
int16_t set_bit_16(int16_t word, uint8_t bit, uint32_t bitPos) {
   bit = bit & 1;
   int16_t mask = 1 << bitPos;
   return (word & ~mask) | (((int16_t)bit) << bitPos);
}
int32_t set_bit_32(int32_t dword, uint8_t bit, uint32_t bitPos) {
   bit = bit & 1;
   int32_t mask = 1 << bitPos;
   return (dword & ~mask) | (((uint32_t)bit) << bitPos);
}
int64_t set_bit_64(int64_t qword, uint8_t bit, uint32_t bitPos) {
   bit = bit & 1;
   int64_t mask = 1ull << bitPos;
   return (qword & ~mask) | (((uint64_t)bit) << bitPos);
}
...
typedef struct UtilTryCheapHashRes {
   bool lenLt13;
   uint64_t hash;
} UtilTryCheapHashRes;
...
#ifdef __aarch64__
   // removing volatile here will emit an instruction, which TPDE does not support yet.
...
typedef struct UtilVarLenRes {
   uint64_t totalEqual;
   uint64_t needsDetailedComp;
} UtilVarLenRes;
