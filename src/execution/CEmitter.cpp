...
LogicalResult printOperation(CppEmitter& emitter, util::CreateConstVarLen op) {
   return printStandardOperation(emitter, op, [&](auto& os) { os << "runtime::VarLen32{reinterpret_cast<const uint8_t*>(R\"RAW(" << (op.getStr().str()) << ")RAW\"), " << op.getStr().size() << ", runtime::StorageClass::GLOBAL}"; });
}
