...
LogicalResult printOperation(CppEmitter& emitter, util::CreateConstVarLen op) {
   return printStandardOperation(emitter, op, [&](auto& os) { os.getOStream() << "runtime::VarLen32{reinterpret_cast<const uint8_t*>(R\"RAW(" << (op.getStr().str()) << ")RAW\"), " << op.getStr().size() << ", runtime::StorageClass::GLOBAL}"; });
}
...
LogicalResult printOperation(CppEmitter& emitter, util::UnPackOp op) {
   auto& os = emitter.ostream();
   if (emitter.emitAssignPrefix(*op).failed()) {
      return failure();
   }
   os << "{";
   for (size_t i = 0; i < op.getVals().size(); i++) {
      os << emitter.getOrCreateName(op.getTuple()) << ".t" << i << (i == op.getVals().size() - 1 ? "}" : ", ");
   }
   os << ";\n";
   return success();
}
...
LogicalResult printOperation(CppEmitter& emitter, util::CreateVarLen op) {
   return printStandardOperation(emitter, op, [&](auto& os) { os << "runtime::VarLen32{reinterpret_cast<uint8_t*>(" << emitter.getOrCreateName(op.getRef()) << "), (uint32_t) " << emitter.getOrCreateName(op.getLen()) << ", runtime::StorageClass::TRANSIENT}"; });
}
...
LogicalResult printOperation(CppEmitter& emitter, util::VarLenInvalid op) {
   if (failed(emitter.emitVariableDeclaration(op->getResult(0), false))) {
      return failure();
   }
   raw_ostream& os = emitter.ostream();
...
LogicalResult printOperation(CppEmitter& emitter, util::VarLenIsInvalid op) {
   if (failed(emitter.emitVariableDeclaration(op->getResult(0), false))) {
      return failure();
   }
   raw_ostream& os = emitter.ostream();
...
LogicalResult printOperation(CppEmitter& emitter, util::PtrTagMatches op) {
   return printStandardOperation(emitter, op, [&](auto& os) { os << "runtime::matchesTag(" << emitter.getOrCreateName(op.getRef()) << "," << emitter.getOrCreateName(op.getHash()) << ")"; });
}
...
LogicalResult printOperation(CppEmitter& emitter, util::BufferGetElementRef op) {
   std::string type;
   if (failed(emitter.typeToString(op->getLoc(), op.getBuffer().getType().getElementType(), type))) {
      return failure();
   }
   return printStandardOperation(emitter, op, [&](auto& os) { os << "&(((" << type << ")" << emitter.getOrCreateName(op.getBuffer()) << ".ptr)[" << emitter.getOrCreateName(op.getIdx()) << "])"; });
}
...
LogicalResult printOperation(CppEmitter& emitter, util::LoadElementOp op) {
   return printStandardOperation(emitter, op, [&](auto& os) { os << emitter.getOrCreateName(op.getRef()) << "->t" << op.getIdx(); });
}
LogicalResult printOperation(CppEmitter& emitter, util::StoreElementOp op) {
   raw_ostream& os = emitter.ostream();
