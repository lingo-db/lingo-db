...
class HtGetRefOrInsertLowering : public OpConversionPattern<mlir::dsa::HashtableGetRefOrInsert> {
   public:
   using OpConversionPattern<mlir::dsa::HashtableGetRefOrInsert>::OpConversionPattern;
   LogicalResult matchAndRewrite(mlir::dsa::HashtableGetRefOrInsert insertOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      ...
      Value currEntryPtr = rewriter.create<util::LoadOp>(loc, bucketPtrType, whileOp.getResult(0));
      Value kvAddress = rewriter.create<util::TupleElementPtrOp>(loc, kvPtrType, currEntryPtr, 2);

      rewriter.replaceOp(insertOp, kvAddress);
...
void populateDSAToStdPatterns(mlir::TypeConverter& typeConverter, mlir::RewritePatternSet& patterns) {
   patterns.insert<CreateDsLowering, HtInsertLowering, HtGetRefOrInsertLowering, FinalizeLowering, DSAppendLowering, LazyJHtInsertLowering, FreeLowering>(typeConverter, patterns.getContext());
