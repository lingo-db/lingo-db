...
struct AggrHashtableBuilder {
   ...
   inline runtime::Pair<bool, runtime::Pointer<uint8_t>> lookup(uint64_t hash) {
...
EXPORT runtime::Pointer<AggrHashtableBuilder> _mlir_ciface_aggr_ht_builder_create(uint64_t keySize, uint64_t valSize, uint64_t aggrSize, uint64_t combinedSize, compareFn compareFunction, updateFn updateFunction, runtime::Pointer<uint8_t>* bytes) { // NOLINT (clang-diagnostic-return-type-c-linkage)
   auto* ptr = new AggrHashtableBuilder(keySize, valSize, aggrSize, combinedSize, compareFunction, updateFunction, (*bytes).get());
   return ptr;
}
EXPORT __attribute__((always_inline)) runtime::ByteRange _mlir_ciface_aggr_ht_builder_add_var_len(runtime::Pointer<AggrHashtableBuilder>* builder, runtime::ByteRange* data) { // NOLINT (clang-diagnostic-return-type-c-linkage)
   (*builder)->currVarLen += (*data).getSize();
   return (*builder)->varLenBuffer.persist(*data);
}
EXPORT __attribute__((always_inline)) runtime::Pair<bool, runtime::ByteRange> _mlir_ciface_aggr_ht_builder_add_nullable_var_len(runtime::Pointer<AggrHashtableBuilder>* builder, bool null, runtime::ByteRange* data) { // NOLINT (clang-diagnostic-return-type-c-linkage)
   if (null) {
...
EXPORT __attribute__((always_inline)) runtime::Pair<bool, runtime::Pointer<uint8_t>> _mlir_ciface_aggr_ht_builder_fast_lookup(runtime::Pointer<AggrHashtableBuilder>* builder, uint64_t hash) { // NOLINT (clang-diagnostic-return-type-c-linkage)
   return (*builder)->lookup(hash);
}
EXPORT __attribute__((always_inline)) void _mlir_ciface_aggr_ht_builder_merge(runtime::Pointer<AggrHashtableBuilder>* builder, uint64_t hash, runtime::Pointer<uint8_t>* key, runtime::Pointer<uint8_t>* val) { // NOLINT (clang-diagnostic-return-type-c-linkage)
   (*builder)->insert(hash, (*key).get(), (*val).get());
}
EXPORT __attribute__((always_inline)) runtime::Pointer<AggrHashtableBuilder> _mlir_ciface_aggr_ht_builder_build(runtime::Pointer<AggrHashtableBuilder>* builder) { // NOLINT (clang-diagnostic-return-type-c-linkage)
   return *builder;
}
using iterator_t = runtime::ObjectBuffer<SimpleHashTable::Entry>::RangeIterator;
EXPORT __attribute__((always_inline)) runtime::Pointer<iterator_t> _mlir_ciface_aggr_ht_iterator_init(runtime::Pointer<AggrHashtableBuilder>* hashmap) { // NOLINT (clang-diagnostic-return-type-c-linkage)
   auto x = (*hashmap)->getHashTable().getBuffer().beginPtr();
   return x;
}
EXPORT __attribute__((always_inline)) runtime::Pointer<iterator_t> _mlir_ciface_aggr_ht_iterator_next(runtime::Pointer<iterator_t>* iterator) { // NOLINT (clang-diagnostic-return-type-c-linkage)
   (*iterator)->operator++();
   return *iterator;
}
EXPORT __attribute__((always_inline)) runtime::Pointer<uint8_t> _mlir_ciface_aggr_ht_iterator_curr(runtime::Pointer<iterator_t>* iterator) { // NOLINT (clang-diagnostic-return-type-c-linkage)
   uint8_t* ptr = (*iterator).ref()->data;
   return ptr;
}

EXPORT __attribute__((always_inline)) bool _mlir_ciface_aggr_ht_iterator_valid(runtime::Pointer<iterator_t>* iterator) {
   return (*iterator)->valid();
}
EXPORT __attribute__((always_inline)) void _mlir_ciface_aggr_ht_iterator_free(runtime::Pointer<iterator_t>* iterator) {
   delete iterator->get();
}