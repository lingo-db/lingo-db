...
EXPORT void _mlir_ciface_sort(runtime::Pointer<Vector>* vector, size_t objSize, bool (*fun_ptr)(uint8_t*, uint8_t*, int64_t, int64_t, int64_t, uint8_t*, uint8_t*, int64_t, uint8_t*, uint8_t*, int64_t), runtime::Pointer<Vector>* res) {
   ...
   std::sort(toSort.begin(), toSort.end(), [&](uint8_t* left, uint8_t* right) {
      return fun_ptr(nullPtr, varLenPtr, 0, varLenBytes, 0, nullPtr, left, 0, nullPtr, right, 0);
   });
